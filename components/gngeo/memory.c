...
Uint8 *fix_usage;
Uint8 sram_lock;
volatile Uint8 sound_code;
volatile Uint8 pending_command;
volatile Uint8 result_code;
Uint16 z80_bank[4];
...
void mem68k_store_invalid_word(Uint32 addr, Uint16 data) {
    if ((addr & 0xFFFFFE) == 0x300000) {
        memory.watchdog = 0; /* Word write to 0x30000x also kicks watchdog */
    } else {
...
#else
            /* Queue NMI for Z80 task on core 1 */
            extern void esp32_z80_queue_nmi(void);
            esp32_z80_queue_nmi();
#endif
        }
    }
