...
void neogeo_main_loop(void) {
	...
	while (!neo_emu_done) {
		...
		/* update the internal representation of keyslot (ESP32 path) */
		update_p1_key();
		update_p2_key();
		update_start();
		update_coin();
...
#ifndef ENABLE_940T
		if (conf.sound) {
			/* Signal Z80 task on core 1 to run frame */
		...
		/* Audio synthesis now handled by Z80 task on core 1 */

		int64_t t68k_start = esp_timer_get_time();
		...
		if (!conf.debug) {
			if (conf.raster) {
				...
				a = neo_interrupt();
				int64_t t_vid_e = esp_timer_get_time();
				...
				{
					static int64_t sum_cpu = 0, sum_vid = 0;
					static int comp_count = 0;
					static int64_t last_comp = 0;
					sum_cpu += (t_cpu_e - t_cpu_s);
					sum_vid += (t_vid_e - t_cpu_e);
					comp_count++;
					if (t_vid_e - last_comp > 1000000) {
						printf("  detail: 68Kcpu=%lld us, video=%lld us (n=%d)\n",
						       sum_cpu / comp_count, sum_vid / comp_count, comp_count);
						sum_cpu = 0; sum_vid = 0; comp_count = 0;
						last_comp = t_vid_e;
					}
				}
			}
		...
		int64_t t68k_end = esp_timer_get_time();

		/* Wait for Z80 frame on core 1 to finish (audio synthesis + I2S) */
		if (conf.sound) {
			extern void esp32_z80_wait_frame(void);
			esp32_z80_wait_frame();
		}
		...
		/* Timing stats every second */
		{
			...
			static int64_t sum_68k = 0, sum_wait = 0;
			static int stat_count = 0;
			int64_t dt68k = t68k_end - t68k_start;
			int64_t dtwait = tz80_end - t68k_end;
			sum_68k += dt68k;
			sum_wait += dtwait;
			stat_count++;
			if (tz80_end - last_stat > 1000000) {
				printf("68K: %lld us/frame, Z80wait: %lld us/frame (n=%d)\n",
				       sum_68k / stat_count, sum_wait / stat_count, stat_count);
				sum_68k = 0; sum_wait = 0; stat_count = 0;
				last_stat = tz80_end;
			}
		}
...
void cpu_68k_dpg_step(void) {
	...
	Uint32 cycle;
	if (nb_cycle == 0) {
		neogeo_main_loop(); /* update event etc. */
	}
