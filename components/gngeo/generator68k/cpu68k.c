...
#ifdef ESP32_PLATFORM
/* These large tables MUST be in PSRAM (~832KB total) */
__attribute__((section(".ext_ram.bss"))) t_iib *cpu68k_iibtable[65536];
__attribute__((section(".ext_ram.bss"))) void (*cpu68k_functable[65536 * 2]) (t_ipc * ipc);
#else
...
uint8 movem_bit[256];
#ifdef ESP32_PLATFORM
__attribute__((section(".ext_ram.bss"))) t_ipclist *ipclist[LEN_IPCLISTTABLE];
#else
