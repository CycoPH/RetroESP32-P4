...
void SuperFXROMMap(void)
{
   ...
   /* SuperFX games always use SRAM for GSU RAM — force SRAMSize if ROM
    * header reports 0 (common for SuperFX ROMs like DOOM). This ensures
    * MAP_LOROM_SRAM_OR_NONE resolves to MAP_LOROM_SRAM, not MAP_NONE. */
   if (Memory.SRAMSize == 0)
      Memory.SRAMSize = 5;  /* 32KB minimum for SuperFX */
   ...
   for (c = 0; c < 0x400; c += 16)
   {
      ...
      Memory.Map [c + 1] = Memory.Map [c + 0x800 + 1] = Memory.RAM;
      Memory.BlockType [c + 0] = Memory.BlockType [c + 0x800] = MAP_TYPE_RAM;
      Memory.BlockType [c + 1] = Memory.BlockType [c + 0x800 + 1] = MAP_TYPE_RAM;

      /* $2000-$3FFF: PPU registers (includes SuperFX regs at $3000+) */
      Memory.Map [c + 2] = Memory.Map [c + 0x802] = (uint8_t*) MAP_PPU;
      Memory.Map [c + 3] = Memory.Map [c + 0x803] = (uint8_t*) MAP_PPU;

      /* $4000-$5FFF: CPU registers */
      Memory.Map [c + 4] = Memory.Map [c + 0x804] = (uint8_t*) MAP_CPU;
      Memory.Map [c + 5] = Memory.Map [c + 0x805] = (uint8_t*) MAP_CPU;

      /* $6000-$7FFF: SRAM (SuperFX backup RAM) */
      Memory.Map [c + 6] = Memory.Map [c + 0x806] = MAP_LOROM_SRAM_OR_NONE;
      Memory.Map [c + 7] = Memory.Map [c + 0x807] = MAP_LOROM_SRAM_OR_NONE;
      Memory.BlockType [c + 6] = Memory.BlockType [c + 0x806] = MAP_TYPE_RAM;
      Memory.BlockType [c + 7] = Memory.BlockType [c + 0x807] = MAP_TYPE_RAM;

      /* $8000-$FFFF: LoROM-style ROM access */
      for (i = c + 8; i < c + 16; i++)
      {
   ...
   /* ---- Banks 40-5F and C0-DF: HiROM-offset linear mapping ---- */
   for (c = 0; c < 0x200; c += 16)
   {
      uint32_t romOffset = ((c >> 4) * 0x10000) % Memory.CalculatedSize;
      for (i = 0; i < 16; i++)
      {
         Memory.Map [c + 0x400 + i] = Memory.Map [c + 0xC00 + i] = &Memory.ROM[romOffset];
         Memory.BlockType [c + 0x400 + i] = Memory.BlockType [c + 0xC00 + i] = MAP_TYPE_ROM;
      }
   }

   /* ---- Banks 60-7D and E0-FD: GSU RAM (SRAM) ---- */
   for (c = 0x600; c < 0x7E0; c += 16)
      for (i = 0; i < 16; i++)
      {
         Memory.Map [c + i] = MAP_LOROM_SRAM_OR_NONE;
         Memory.BlockType [c + i] = MAP_TYPE_RAM;
      }
   for (c = 0xE00; c < 0xFE0; c += 16)
      for (i = 0; i < 16; i++)
      {
         Memory.Map [c + i] = MAP_LOROM_SRAM_OR_NONE;
         Memory.BlockType [c + i] = MAP_TYPE_RAM;
   ...
   /* ---- Bank 70-71: direct SRAM for GSU RAM access ----
    * Must be AFTER MapRAM() because MapRAM() overwrites bank 70+ with
    * MAP_LOROM_SRAM_OR_NONE which returns MAP_NONE if SRAMSize==0. */
   if (Memory.SRAM)
   {
      for (i = 0; i < 16; i++)
      {
         Memory.Map [0x700 + i] = Memory.SRAM;
         Memory.Map [0x710 + i] = Memory.SRAM + 0x10000;
         Memory.BlockType [0x700 + i] = MAP_TYPE_RAM;
         Memory.BlockType [0x710 + i] = MAP_TYPE_RAM;
      }
   }
}
...
bool S9xInitMemory(void)
{
   ...
   Memory.SRAM  = (uint8_t*) heap_caps_calloc(1, SRAM_SIZE, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);  // 128 KB -- DRAM for GSU framebuffer write performance
