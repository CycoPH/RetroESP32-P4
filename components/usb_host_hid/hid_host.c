...
static esp_err_t hid_host_device_disconnected(usb_device_handle_t dev_hdl)
{
    ...
    while (hid_iface_curr != NULL) {
        ...
        if (hid_iface_curr->parent && (hid_iface_curr->parent->dev_addr == hid_device->dev_addr)) {
            ...
            if (close_err != ESP_OK) {
                ESP_LOGW(TAG, "device_close returned %s for iface %d, continuing",
                         esp_err_to_name(close_err), hid_iface_curr->dev_params.iface_num);
            }
        }
        HID_ENTER_CRITICAL();
    ...
    if (uninstall_err != ESP_OK) {
        ESP_LOGW(TAG, "uninstall_device returned %s", esp_err_to_name(uninstall_err));
    }
...
static esp_err_t hid_host_interface_release_and_free_transfer(hid_iface_t *iface)
{
    ...
    /* Try to release the interface — may fail if device disconnected.
     * Proceed with cleanup regardless to avoid resource leaks. */
    esp_err_t release_err = usb_host_interface_release(s_hid_driver->client_handle,
                                                       iface->parent->dev_hdl,
                                                       iface->dev_params.iface_num);
    if (release_err != ESP_OK) {
        ESP_LOGW(TAG, "Interface release returned %s (device may have disconnected), continuing cleanup",
    ...
    return release_err;  /* Return original error but cleanup is done */
}
...
static esp_err_t hid_host_disable_interface(hid_iface_t *iface)
{
    ...
    if (HID_INTERFACE_STATE_ACTIVE == iface->state) {
        /* Endpoint halt/flush may fail if device disconnected — that's OK,
         * we still need to update state and release resources. */
        esp_err_t err;
        err = usb_host_endpoint_halt(iface->parent->dev_hdl, iface->ep_in);
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "EP halt returned %s (device may be gone)", esp_err_to_name(err));
        }
        err = usb_host_endpoint_flush(iface->parent->dev_hdl, iface->ep_in);
        if (err != ESP_OK) {
...
static void in_xfer_done(usb_transfer_t *in_xfer)
{
    ...
    case USB_TRANSFER_STATUS_COMPLETED:
        ...
        // Relaunch transfer
        {
            ...
            if (err != ESP_OK) {
                /* Resubmit failed — most likely the port disconnected.
                 * Do NOT try halt/flush/clear here — the port may be in RECOVERY state
                 * and those commands would fail, leaving URBs in-flight and preventing
                 * interface release later.
                 * Simply stop resubmitting. The USB Host Library will detect the
                 * disconnect, cancel pending URBs, and fire DEV_GONE → DISCONNECTED
                 * through the normal flow, which will handle cleanup properly. */
                ESP_LOGW(TAG, "IN resubmit failed (%s), stopping polling (will disconnect via USB core)",
                         esp_err_to_name(err));
            }
        }
        return;
...
esp_err_t hid_host_uninstall_device(hid_device_t *hid_device)
{
    ...
    /* Transfer free and device close may fail if device disconnected,
     * but we must continue cleanup to avoid resource leaks */
    esp_err_t err;
    err = usb_host_transfer_free(hid_device->ctrl_xfer);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "transfer_free returned %s, continuing", esp_err_to_name(err));
    }
    hid_device->ctrl_xfer = NULL;

    err = usb_host_device_close(s_hid_driver->client_handle,
                                hid_device->dev_hdl);
    if (err != ESP_OK) {
        ESP_LOGW(TAG, "usb_host_device_close returned %s, continuing cleanup", esp_err_to_name(err));
    }
...
esp_err_t hid_host_device_close(hid_host_device_handle_t hid_dev_handle)
{
    ...
    if (HID_INTERFACE_STATE_ACTIVE == hid_iface->state ||
        ...
        /* disable may fail if device disconnected — continue anyway */
        esp_err_t dis_err = hid_host_disable_interface(hid_iface);
        if (dis_err != ESP_OK) {
            ESP_LOGW(TAG, "disable_interface returned %s, forcing state to READY", esp_err_to_name(dis_err));
            hid_iface->state = HID_INTERFACE_STATE_READY;
        }
    }

    if (HID_INTERFACE_STATE_READY == hid_iface->state) {
        /* release may fail if device disconnected — continue anyway */
        esp_err_t rel_err = hid_host_interface_release_and_free_transfer(hid_iface);
        if (rel_err != ESP_OK) {
            ESP_LOGW(TAG, "release_and_free returned %s, continuing close", esp_err_to_name(rel_err));
        }
